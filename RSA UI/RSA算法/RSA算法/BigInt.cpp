...
//Construct large number object and initialize to zero
CBigInt::CBigInt()
{
    ZeroData(MAX_BI_LEN);
}
...
CBigInt CBigInt::operator-(const CBigInt& value) const
{
    ...
    else
    {
        ...
        {
            CBigInt::Sub(value, *this, r);
            r.m_Sign = (m_Sign == 0) ? 1 : 0; //Need to change sign
        }
    }
...
void CBigInt::RemRight(unsigned int length)
{
    if(m_nLength > length)
    {
        for(unsigned int i = length; i < m_nLength; i++)
        {
            m_ulValue[i] = 0;
        }
        m_nLength = length;
...
void CBigInt::TrimRight(unsigned int length)
{
    if(m_nLength <= length)
    {
        ZeroData(m_nLength);
    ...
    unsigned int left = m_nLength - length;
    for(unsigned int i = 0; i < m_nLength; i++)
    {
        if(i < left)
        {
            m_ulValue[i] = m_ulValue[i + length];
        }
        else
        {
            m_ulValue[i] = 0;
...
void CBigInt::Add(const CBigInt& value1, const CBigInt& value2, CBigInt& result)
{
    ...
    unsigned carry = 0;
    /*Adjust the number of digits first*/
    ...
    //Deal with the highest digit, if the current highest digit carries carry! = 0, you need to increase the number of digits
...
void CBigInt::Mul(const CBigInt& value1, const CBigInt& value2, CBigInt& result)
{
    ...
    CBigInt r;
	r.m_nLength = value1.m_nLength + value2.m_nLength - 1; //Number of digits in preliminary estimates
    for(unsigned int i = 0; i < r.m_nLength; i++)
	{
	...
	if(carry != 0) //Finally, there is still a carry, then the large digits need to be expanded
    {
        r.m_nLength++;
